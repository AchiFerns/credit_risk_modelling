...
'''Takes in raw values directly from your app’s form (age, income, loan details, categorical fields, etc.).
These inputs are not yet in the right shape/scale/format for the model.'''
...
User enters values → you build a dictionary with raw + computed + dummy values.
Convert to DataFrame.
Scale numeric features using the saved scaler.
Reorder columns so they match training exactly.
...
def predict(age, income, loan_amount, loan_tenure_months, avg_dpd_per_delinquency,
    ...
    '''Takes all user inputs directly from your UI form in main.py.

Parameters match exactly what the user enters: age, income, loan details, ratios, categorical options.'''
    # Prepare input data
